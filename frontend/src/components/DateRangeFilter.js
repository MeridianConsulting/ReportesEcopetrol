...
export default function DateRangeFilter({ onChange, defaultPeriod = 'today', valueFrom, valueTo }) {
  ...
  // Sincronizar con el período del padre (para que "Todo" y "Personalizado" no se pierdan al recargar)
  useEffect(() => {
    const option = PERIOD_OPTIONS.find(o => o.id === defaultPeriod);
    setSelectedPeriod(defaultPeriod);
    if (defaultPeriod === 'custom' && valueFrom && valueTo) {
      setDateFrom(valueFrom);
      setDateTo(valueTo);
    } else if (option && option.getValue) {
  ...
  }, [defaultPeriod, valueFrom, valueTo]);
