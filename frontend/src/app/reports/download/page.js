...
export default function ReportsDownload() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // Periodo actual para KPIs
      const now = new Date();
      const periodKey = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
      
      const [areasData, tasksData, usersData, reportData, kpiSummary] = await Promise.all([
        apiRequest('/areas'),
        apiRequest(tasksUrl),
        apiRequest('/users'),
        apiRequest(reportUrl),
        user?.role === 'admin' ? apiRequest(`/kpis/summary/all?period=${periodKey}`).catch(() => null) : Promise.resolve(null)
      ]);
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      // Determinar tipo de reporte y datos
      let data, title;
      if (reportType === 'kpis') {
        data = getGeneralData(); // Para estadísticas generales
        title = 'Reporte de Indicadores KPI';
      } else if (reportType === 'general') {
        data = getGeneralData();
        title = 'Reporte General de Tareas';
      } else {
        data = getAreaData(selectedAreaId);
        title = `Reporte de ${data.area?.name || 'Area'}`;
      }
      ...
      doc.text('Production Analytics Reports', margin, 26);
      ...
      // === SECCIÓN DE KPIs (para reporte de KPIs o general con datos KPI) ===
      if ((reportType === 'kpis' || reportType === 'general') && kpiData?.data && kpiData.data.length > 0) {
        if (y > pageHeight - 80) {
          doc.addPage();
          y = margin;
        }
        ...
        doc.setFontSize(10);
        doc.setFont('helvetica', 'bold');
        doc.text('Indicadores KPI del Sistema', margin, y);
        y += 6;

        // Resumen global de KPIs
        const kpiStats = kpiData.global_stats || {};
        const kpiSummaryWidth = (pageWidth - margin * 2 - 15) / 4;
        const kpiSummary = [
          { label: 'Promedio', value: `${kpiStats.avg_performance || 0}%`, color: primaryColor },
          { label: 'En Meta', value: `${kpiStats.green_count || 0}`, color: greenColor },
          { label: 'Precaucion', value: `${kpiStats.yellow_count || 0}`, color: amberColor },
          { label: 'Critico', value: `${kpiStats.red_count || 0}`, color: roseColor },
        ];

        kpiSummary.forEach((kpi, i) => {
          const x = margin + i * (kpiSummaryWidth + 5);
          doc.setDrawColor(...kpi.color);
          doc.setFillColor(248, 250, 252);
          doc.roundedRect(x, y, kpiSummaryWidth, 16, 2, 2, 'FD');
          
          doc.setTextColor(...kpi.color);
          doc.setFontSize(12);
          doc.setFont('helvetica', 'bold');
          doc.text(kpi.value, x + kpiSummaryWidth / 2, y + 7, { align: 'center' });
          
          doc.setTextColor(...slateColor);
          doc.setFontSize(7);
          doc.setFont('helvetica', 'normal');
          doc.text(kpi.label, x + kpiSummaryWidth / 2, y + 13, { align: 'center' });
        });
        ...
        doc.setFont('helvetica', 'bold');
        doc.text('AREA', margin + 2, y + 5);
        doc.text('KPIS', margin + 55, y + 5);
        doc.text('EN META', margin + 75, y + 5);
        doc.text('PRECAUCION', margin + 100, y + 5);
        doc.text('CRITICO', margin + 130, y + 5);
        doc.text('% CUMPL.', margin + 155, y + 5);
        y += 8;

        kpiData.data.slice(0, 10).forEach((area, i) => {
          if (y > pageHeight - 15) {
            doc.addPage();
            y = margin;
          }

          const stats = area.stats || { green: 0, yellow: 0, red: 0, na: 0 };
          const totalActive = stats.green + stats.yellow + stats.red;
          const cumplimiento = totalActive > 0 
          ...
          doc.setFontSize(8);
          doc.setFont('helvetica', 'normal');
          doc.text((area.area_name || '').substring(0, 22), margin + 2, y + 3);
          doc.text(`${area.total_kpis || 0}`, margin + 55, y + 3);
          
          doc.setTextColor(...greenColor);
          doc.text(`${stats.green}`, margin + 75, y + 3);
          
          doc.setTextColor(...amberColor);
          doc.text(`${stats.yellow}`, margin + 100, y + 3);
          
          doc.setTextColor(...roseColor);
          doc.text(`${stats.red}`, margin + 130, y + 3);
          
          doc.setTextColor(...(cumplimiento >= 80 ? greenColor : cumplimiento >= 60 ? amberColor : roseColor));
          doc.setFont('helvetica', 'bold');
          doc.text(`${cumplimiento}%`, margin + 155, y + 3);
      ...
      // === TABLA DE TAREAS (omitir para reporte solo de KPIs) ===
      if (reportType !== 'kpis') {
        if (y > pageHeight - 50) {
          doc.addPage();
          y = margin;
      ...
      doc.text('Production Analytics Reports', pageWidth - margin, pageHeight - 10, { align: 'right' });

      // Guardar
      const dateSuffix = (dateFrom && dateTo) ? `${dateFrom}_${dateTo}` : 'todas_fechas';
      let fileName;
      if (reportType === 'kpis') {
        fileName = `Reporte_KPIs_${dateSuffix}.pdf`;
      } else if (reportType === 'general') {
        fileName = `Reporte_General_${dateSuffix}.pdf`;
      } else {
        fileName = `Reporte_${data.area?.name || 'Area'}_${dateSuffix}.pdf`;
      }
  ...
  const generateExcel = async () => {
    ...
    try {
      ...
      subtitleCell.value = `Production Analytics Reports | Período: ${getPeriodLabel()} | Generado: ${new Date().toLocaleDateString('es-ES')}`;
      ...
      // ==========================================
      // HOJA 3: KPIs (si hay datos de KPIs)
      // ==========================================
      if ((reportType === 'kpis' || reportType === 'general') && kpiData?.data && kpiData.data.length > 0) {
        const kpisSheet = workbook.addWorksheet('Indicadores KPI', {
          properties: { tabColor: { argb: '7C3AED' } }
        });
        
        kpisSheet.columns = [
          { width: 30 }, { width: 12 }, { width: 12 }, { width: 14 }, { width: 12 }, { width: 14 }
        ];

        // Título
        kpisSheet.mergeCells('A1:F1');
        const kpisTitleCell = kpisSheet.getCell('A1');
        kpisTitleCell.value = '📊 INDICADORES KPI DEL SISTEMA';
        kpisTitleCell.font = { size: 18, bold: true, color: { argb: 'FFFFFF' } };
        kpisTitleCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: '7C3AED' } };
        kpisTitleCell.alignment = { horizontal: 'center', vertical: 'middle' };
        kpisSheet.getRow(1).height = 35;

        // Resumen global
        const globalStats = kpiData.global_stats || {};
        kpisSheet.mergeCells('A3:F3');
        kpisSheet.getCell('A3').value = 'Resumen Global';
        kpisSheet.getCell('A3').font = { size: 12, bold: true, color: { argb: '1E293B' } };
        
        const kpiGlobalRow = ['', 'Promedio', 'En Meta', 'Precaución', 'Crítico', 'Sin Datos'];
        const kpiGlobalVals = ['', `${globalStats.avg_performance || 0}%`, globalStats.green_count || 0, globalStats.yellow_count || 0, globalStats.red_count || 0, globalStats.na_count || 0];
        const kpiGlobalColors = ['', primaryColor, greenColor, amberColor, roseColor, slateColor];
        
        kpisSheet.getRow(4).values = kpiGlobalRow;
        kpisSheet.getRow(4).font = { bold: true, color: { argb: 'FFFFFF' } };
        for (let j = 2; j <= 6; j++) {
          const cell = kpisSheet.getCell(4, j);
          cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: kpiGlobalColors[j] } };
          cell.alignment = { horizontal: 'center' };
        }
        
        kpisSheet.getRow(5).values = kpiGlobalVals;
        kpisSheet.getRow(5).font = { bold: true, size: 14 };
        for (let j = 2; j <= 6; j++) {
          const cell = kpisSheet.getCell(5, j);
          cell.font = { bold: true, size: 14, color: { argb: kpiGlobalColors[j] } };
          cell.alignment = { horizontal: 'center' };
          cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: lightGray } };
        }

        // Headers de tabla por área
        kpisSheet.getRow(7).values = ['Área', 'KPIs', 'En Meta', 'Precaución', 'Crítico', '% Cumplimiento'];
        kpisSheet.getRow(7).font = { bold: true, color: { argb: 'FFFFFF' } };
        kpisSheet.getRow(7).height = 25;
        for (let j = 1; j <= 6; j++) {
          const cell = kpisSheet.getCell(7, j);
          cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: primaryColor } };
          cell.alignment = { horizontal: 'center', vertical: 'middle' };
        }

        // Datos por área
        kpiData.data.forEach((area, i) => {
          const rowNum = 8 + i;
          const stats = area.stats || { green: 0, yellow: 0, red: 0, na: 0 };
          const totalActive = stats.green + stats.yellow + stats.red;
          const cumplimiento = totalActive > 0 
          ...
          kpisSheet.getRow(rowNum).values = [
            area.area_name || '',
            area.total_kpis || 0,
            stats.green,
            stats.yellow,
          ...
          kpisSheet.getRow(rowNum).height = 22;
          
          for (let j = 1; j <= 6; j++) {
            const cell = kpisSheet.getCell(rowNum, j);
            cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: i % 2 === 0 ? lightGray : 'FFFFFF' } };
            if (j > 1) cell.alignment = { horizontal: 'center' };
          }
          
          // Colores especiales
          kpisSheet.getCell(rowNum, 3).font = { bold: true, color: { argb: greenColor } };
          kpisSheet.getCell(rowNum, 4).font = { bold: true, color: { argb: amberColor } };
          kpisSheet.getCell(rowNum, 5).font = { bold: true, color: { argb: roseColor } };
          kpisSheet.getCell(rowNum, 6).font = { 
            bold: true, 
            color: { argb: cumplimiento >= 80 ? greenColor : cumplimiento >= 60 ? amberColor : roseColor } 
          };
        });
      }

      // ==========================================
      // HOJA 4: DETALLE DE TAREAS (omitir para reporte solo de KPIs)
      // ==========================================
      if (reportType !== 'kpis') {
      ...
      let fileName;
      if (reportType === 'kpis') {
        fileName = `Reporte_KPIs_${dateSuffix}.xlsx`;
      } else if (reportType === 'general') {
        fileName = `Reporte_General_${dateSuffix}.xlsx`;
      } else {
        fileName = `Reporte_${data.area?.name || 'Area'}_${dateSuffix}.xlsx`;
      }
  ...
  const getReportData = () => {
    if (reportType === 'kpis') return { total: kpiData?.data?.length || 0, tasks: [] };
    if (reportType === 'general') return getGeneralData();
    return selectedAreaId ? getAreaData(selectedAreaId) : null;
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              >
                ...
                {user?.role === 'admin' && <option value="kpis">Reporte de KPIs</option>}
              </select>
            </div>
            ...
            <div className="flex items-end gap-2 flex-wrap">
              <button
                ...
                disabled={loading || (reportType === 'area' && !selectedAreaId) || (reportType === 'kpis' && (!kpiData || !kpiData.data))}
              ...
              <button
                ...
                disabled={generatingExcel || loading || (reportType === 'area' && !selectedAreaId) || (reportType === 'kpis' && (!kpiData || !kpiData.data))}
        ...
        {showPreview && data && (
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            ...
            <div className="p-6">
              ...
              <div className="border-b-2 border-green-600 pb-4 mb-6">
                <h1 className="text-2xl font-bold text-slate-900">
                  {reportType === 'kpis' ? 'Reporte de Indicadores KPI' : 
                   ...
                   `Reporte de ${data.area?.name || 'Area'}`}
                </h1>
                <div className="flex items-center justify-between mt-2">
                  <p className="text-slate-600">Production Analytics Reports</p>
              ...
              {/* Vista previa de KPIs (para reporte de KPIs) */}
              {reportType === 'kpis' && kpiData?.data && (
                <>
                  {/* Resumen global de KPIs */}
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
                    ...
                    <div className="bg-emerald-50 rounded-lg p-4 border border-emerald-200">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-2xl font-bold text-emerald-700">{kpiData.global_stats?.green_count || 0}</p>
                          <p className="text-xs text-emerald-600">En Meta</p>
                        </div>
                      </div>
                    </div>
                    <div className="bg-amber-50 rounded-lg p-4 border border-amber-200">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-2xl font-bold text-amber-700">{kpiData.global_stats?.yellow_count || 0}</p>
                          <p className="text-xs text-amber-600">Precaucion</p>
                        </div>
                      </div>
                    </div>
                    <div className="bg-rose-50 rounded-lg p-4 border border-rose-200">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-2xl font-bold text-rose-700">{kpiData.global_stats?.red_count || 0}</p>
                          <p className="text-xs text-rose-600">Critico</p>
                        </div>
                      </div>
                    </div>
                    <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-2xl font-bold text-slate-700">{kpiData.global_stats?.na_count || 0}</p>
                      ...
                      <table className="w-full text-sm">
                        <thead className="bg-slate-100">
                          <tr>
                            ...
                            <th className="px-3 py-2 text-center text-xs font-semibold text-slate-600">KPIs</th>
                            <th className="px-3 py-2 text-center text-xs font-semibold text-slate-600">En Meta</th>
                            <th className="px-3 py-2 text-center text-xs font-semibold text-slate-600">Precaucion</th>
                            <th className="px-3 py-2 text-center text-xs font-semibold text-slate-600">Critico</th>
                            <th className="px-3 py-2 text-center text-xs font-semibold text-slate-600">% Cumpl.</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                          {kpiData.data.map(area => {
                            const stats = area.stats || { green: 0, yellow: 0, red: 0 };
                            const totalActive = stats.green + stats.yellow + stats.red;
                            const cumplimiento = totalActive > 0 
                              ? Math.round(((stats.green * 100) + (stats.yellow * 50)) / totalActive)
                              : 0;
                            return (
                              <tr key={area.area_id}>
                                ...
                                <td className="px-3 py-2 text-center text-slate-600">{area.total_kpis || 0}</td>
                                <td className="px-3 py-2 text-center text-emerald-600 font-medium">{stats.green}</td>
                                <td className="px-3 py-2 text-center text-amber-600 font-medium">{stats.yellow}</td>
                                <td className="px-3 py-2 text-center text-rose-600 font-medium">{stats.red}</td>
                                <td className="px-3 py-2 text-center">
                                  <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                                    cumplimiento >= 80 ? 'bg-emerald-100 text-emerald-700' :
                                    cumplimiento >= 60 ? 'bg-amber-100 text-amber-700' :
              ...
              {/* KPIs de tareas (para reporte general y por área) */}
              {reportType !== 'kpis' && (
                <>
              ...
              {/* Lista de tareas (solo para reportes que no son KPIs) */}
              {reportType !== 'kpis' && (
                <div>
                  <h3 className="font-semibold text-slate-900 mb-3 flex items-center gap-2">
